...
<form id="yith-wcwl-form" action="<?php echo esc_url( YITH_WCWL()->get_wishlist_url( 'manage' ) ) ?>" method="post">
    ...
    <table class="shop_table cart wishlist_table wishlist_manage_table" cellspacing="0" >
                ...
                <?php foreach( $user_wishlists as $wishlist ): ?>
                    ...
                    <tr>
                        ...
                        <td class="wishlist-delete">
	                        ...
	                        <a class="button btn btn-ghost" href="<?php echo esc_url( add_query_arg( 'wishlist_id', $wishlist['ID'], wp_nonce_url( YITH_WCWL()->get_wishlist_url( 'manage' ), 'yith_wcwl_delete_action', 'yith_wcwl_delete' ) ) ) ?>"><?php _e( 'Delete', 'yith-woocommerce-wishlist' ) ?></a>
                        </td>
                    </tr>
                    <?php
		                $wishlist_count ++;
        ...
        <tfoot>
            <tr>
                <td colspan="3">
                    <a class="btn btn-flat-green button create-new-wishlist" href="<?php echo YITH_WCWL()->get_wishlist_url( 'create' ) ?>"><?php _e( 'Create a new wishlist', 'yith-woocommerce-wishlist' )?></a>
                    <?php if( ! empty( $user_wishlists ) && $wishlist_count ): ?>
                        <button class="btn-ghost submit-wishlist-changes"><?php _e( 'Save Settings', 'yith-woocommerce-wishlist' ) ?></button>
                    <?php endif; ?>
                </td>
            </tr>
        </tfoot>
