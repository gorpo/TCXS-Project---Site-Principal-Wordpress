...
class YIT_Layout_Options {
    ...
    public function get_option( $key, $id = false, $type = 'post', $model = 'post_type' ) {
        ...
        if( function_exists( 'is_product_taxonomy' ) && function_exists( 'is_product_category' ) ){
            //In order to works with ajax product filter
            $is_product_tax = is_product_taxonomy() && is_product_category();
        }

        $is_product_tax = apply_filters( 'yit_layout_option_is_product_tax', $is_product_tax );
        ...
        if ( !$id ) {
            ...
            elseif ( $wp->is_tax() && $is_product_tax ) {
