...
if ( function_exists( 'YIT_Slider' ) && ! in_array( YIT_Layout()->slider_name, array( '', 'none' ) ) ){
	$slider = YIT_Slider::get_slider( YIT_Layout()->slider_name );

	if( isset( $slider->config ) && isset( $slider->config->layout ) ) {
		$opened = $opened && ($slider->config->layout != 'banners');
	}
}
...
<div class="shop-by-category nav vertical<?php if ( $opened ) echo ' opened '.$can_close_class ?>">
