...
<div class="testimonials-slider <?php esc_attr( $vc_css ); ?>">
    <?php if( isset( $title_text ) && ! empty( $title_text ) ) echo do_shortcode( str_replace( '%content%', $title_text, $title_shortcode ) ); ?>
    <?php if ( $is_slider ): ?>
        <ul class="testimonial-content owl-slider hide-elem" data-slidespeed = "<?php echo esc_attr( $speed ) ?>" data-dots = "<?php echo $pagination ?>" data-dotsspeed = "<?php echo esc_attr( $paginationspeed ) ?>" data-navigation = "<?php echo $navigation ?>" data-autoplay = "<?php echo $autoplay ?>" data-singleitem = "true" >
    <?php else: ?>
